...
const FORTIFY_API_TOKEN = process.env.FORTIFY_API_TOKEN;
...
async function main() {
    const tokenData = await fetch('https://api.ams.fortify.com/oauth/token', {
        ...
        body: `grant_type=password&scope=api-tenant&username=${FORTIFY_TENANT}\\${FORTIFY_USER}&password=${FORTIFY_API_TOKEN}&security_code=`
    }).then(r => r.json());
